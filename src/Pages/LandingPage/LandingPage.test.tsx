...
test("should change default currency", async() => {
  const { getByTestId } = render(<GlobalProvider><LandingPage /></GlobalProvider>);
...
test("floating menu", async() => {
  const { getByTestId, asFragment } = render(<GlobalProvider><LandingPage /></GlobalProvider>);
  
  const landingPage = getByTestId("landing-page");
  const transactionHistory = landingPage.getElementsByClassName("floating-menu").item(0)?.getElementsByClassName("item").item(0);
  const addNewCurrency = landingPage.getElementsByClassName("floating-menu").item(0)?.getElementsByClassName("item").item(1);
